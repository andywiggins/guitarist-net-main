...
class MonoNetworkSmall(nn.Module):
    ...
    def __init__(self, 
        ...
        use_mfcc_input : bool
            if true, then network expects mfcc as additional input 
        use_stft_input : bool
            if true, then network expects stft as additional input 
        ...
        # optionally include stft as an input
        self.use_stft_input = use_stft_input
        if self.use_stft_input:
            self.num_inputs += (N_FFT // 2) + 1 # this is the number of freq bins in the stft
    ...
    def forward(self, conditioning, string_idx=None, mfcc=None, stft=None, context=None):
        """
        ...
        mfcc : tensor (batch, frames, n_mfcc)
            mfcc of input audio
        stft : tensor (batch, frames, freq_bins)
        ...
        # if using stft, concatenate onto inputs
        if self.use_stft_input:
            inputs = torch.cat([inputs, stft], -1)
        ...
        if MASK_STRING_DATA_WITH_CONDITIONING:
            ...
            # harm_distr = harm_distr * string_mask[:, None, :] # (batch, # harmonics, frames) ## don't mask these as it causes nan in normalization later, overall amplitude being masked should be enough.
