...
from midi_synth.residual_synth_network import ResidualSynthNetwork
from midi_synth.residual_synth_unconstrained import ResidualSynthUnconstrained
...
class MidiSynth(nn.Module):
    ...
    def __init__(self,
                ...
                use_stft_input=MIDI_SYNTH_USE_STFT_INPUT,
                ...
                use_residual_synth=USE_RESIDUAL_SYNTH,
                residual_synth_mode=RESIDUAL_SYNTH_MODE,
        ...
        use_stft_input : bool
            if true, then synth expects audio as input so that stfts can be calculated
        ...
        use_context_net : bool
            if true use a context network
        use_residual_synth : bool
            if true use a residual synth
        residual_synth_mode : string
            "stft" for unconstrained spectrogram, "ddsp" for f0 and amplitude params
        reverb_length : int
        ...
        self.use_stft_input = use_stft_input
        if self.use_stft_input:
            print("Using stft as additional input to synth.")
            self.config['inputs'] += [target_audio] # add "mic_audio" to inputs, for instance, so that stft can be calculated

            self.stft_transform = torchaudio.transforms.Spectrogram(n_fft=N_FFT, hop_length=HOP_LENGTH)

        if use_small_mono_net:
            self.mono_network = MonoNetworkSmall(
                                linear_1_size=sm_linear_1_size,
                                linear_2_size=sm_linear_2_size,
        ...
        # optional gain before the reverb
        if MIDI_SYNTH_GAIN_PARAM:
            self.gain = nn.Parameter(
                torch.tensor([40.0], dtype=torch.float32), requires_grad=True
            )

        self.use_residual_synth = use_residual_synth
        self.residual_synth_mode = residual_synth_mode
        if self.use_residual_synth:
            if residual_synth_mode == "ddsp":
                self.residual_synth_network = ResidualSynthNetwork(
                                    linear_1_size=sm_linear_1_size,
                                    linear_2_size=sm_linear_2_size,
                                    gru_num_units=sm_gru_num_units,
                                    num_oscillators=num_oscillators,
                                    num_filter_coeffs=num_filter_coeffs,
                                    use_mfcc_input=use_mfcc_input)
            elif residual_synth_mode == "stft":
                self.residual_synth = ResidualSynthUnconstrained(
    ...
    def forward(self, conditioning, input_audio=None):
        ...
        if self.use_mfcc_input:
            mfcc = crop_or_pad(self.mfcc_transform(input_audio), num_frames) # crop or pad the mfcc to be the right number of frames
            mfcc = torch.transpose(mfcc, 1, 2) # (batch, frames, n_mfcc) # put n_mfcc last since the linear layers assume channel last
        else:
            mfcc = None

        # calculated stft from input audio
        if self.use_stft_input:
            stft = crop_or_pad(self.stft_transform(input_audio), num_frames) # crop or pad the stft to be the right number of frames
            stft = torch.transpose(stft, 1, 2) # (batch, frames, n_fft) # put n_fft last since the linear layers assume channel last
        else:
            stft = None
        ...
        for i in range(6):
            ...
            params = self.mono_network(string_cond, string_idx=string_idx, mfcc=mfcc, stft=stft, context=context)       
        ...
        if self.use_residual_synth and self.residual_synth_mode=="ddsp":
            residual_params = self.residual_synth_network(conditioning, mfcc)
            residual_harmonic = self.harmonic_oscillator(residual_params) # (batch, samples)
            residual_noise = self.filtered_noise(residual_params) # (batch, samples)
            residual_audio = residual_harmonic + residual_noise # (batch, samples)
        elif self.use_residual_synth and self.residual_synth_mode=="stft":
            residual_audio = self.residual_synth(conditioning, mfcc=mfcc)
            residual_audio = crop_or_pad(residual_audio, dry_audio.shape[-1]) # make audio length match (batch, samples)
        else:
            residual_audio = 0

        if MIDI_SYNTH_GAIN_PARAM:
            self.gain.to(dry_audio.device)
            dry_audio *= self.gain

        # if the model is either in training mode or we do not want to bypass residual during eval
        if self.training or (not BYPASS_RESIDUAL_DURING_EVAL):
            audio = self.reverb(dry_audio + residual_audio)
        else:
            audio = self.reverb(dry_audio)

        outputs = {}
        outputs['dry_audio'] = dry_audio
        outputs['audio'] = audio

        if self.use_residual_synth:
            outputs['residual_audio'] = residual_audio
    ...
    def forward_parallel(self, conditioning, input_audio=None):
        ...
        # optional: calculate mfcc from input audio
        if self.use_mfcc_input:
            mfcc = crop_or_pad(self.mfcc_transform(input_audio), num_frames)  # Crop or pad to get correct num frames. Shape: (batch, n_mfcc, frames)
            mfcc_batch = torch.transpose(mfcc, 1, 2) # (batch, frames, n_mfcc) # put n_mfcc last since the linear layers assume channel last
            mfcc_batch6 = mfcc_batch.unsqueeze(0).expand(6, -1, -1, -1).flatten(0,1) # expand batch dim to size=(batch*6)
        else:
            mfcc_batch = None
            mfcc_batch6 = None
        
        # optional: calculate stft from input audio
        if self.use_stft_input:
            stft = crop_or_pad(self.stft_transform(input_audio), num_frames)  # Crop or pad to get correct num frames. Shape: (batch, n_fft, frames)
            stft_batch = torch.transpose(stft, 1, 2) # (batch, frames, n_fft) # put n_fft last since the linear layers assume channel last
            stft_batch6 = stft_batch.unsqueeze(0).expand(6, -1, -1, -1).flatten(0,1) # expand batch dim to size=(batch*6)
        else:
            stft_batch = None
            stft_batch6 = None

        # optional: before the conditioning is reshaped for parallelization, plug into residual synth network
        if self.use_residual_synth and self.residual_synth_mode=="ddsp":
            residual_params = self.residual_synth_network(conditioning, mfcc_batch)
            residual_harmonic = self.harmonic_oscillator(residual_params) # (batch, samples)
            residual_noise = self.filtered_noise(residual_params) # (batch, samples)
            residual_audio = residual_harmonic + residual_noise # (batch, samples)
        elif self.use_residual_synth and self.residual_synth_mode=="stft":
            residual_audio = self.residual_synth(conditioning, mfcc=mfcc_batch)
        else:
            residual_audio = 0
        ...
        # parallelize guitar strings along with batch
        ...
        params = self.mono_network(conditioning, string_idx=string_idx, mfcc=mfcc_batch6, stft=stft_batch6, context=context)       
        ...
        # if using residual and the model is either in training mode or we do not want to bypass residual during eval
        if self.use_residual_synth and (self.training or (not BYPASS_RESIDUAL_DURING_EVAL)):
            residual_audio = crop_or_pad(residual_audio, dry_audio.shape[-1])
            audio = self.reverb(dry_audio + residual_audio)
        else:
            audio = self.reverb(dry_audio)
        ...
        if self.use_residual_synth:
            outputs['residual_audio'] = residual_audio
            if self.residual_synth_mode == "ddsp":
                outputs['residual_params'] = residual_params
