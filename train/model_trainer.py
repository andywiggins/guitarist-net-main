...
class ModelTrainer:
    ...
    def __init__(self, model, train_dataset, test_dataset,
        ...
        self.min_validation_loss_epoch = 0

        self.min_training_loss = np.inf
        self.min_training_loss_epoch = 0
    ...
    def load_checkpoint(self, output_path=OUTPUT_PATH, experiment_name=None, checkpoint_name=None, checkpoint_path=None, load_latest=True, strict=True):
        """
        ...
        strict : bool
            param for model.load_state_dict
            if true, model params must match exactly
            if false, nonmatching state_dict keys are ignored and optimizer state dict is not loaded
        """
        ...
        self.model.load_state_dict(loaded_checkpoint['model_state_dict'], strict=strict)
        if strict:
        ...
        # set the minimum validation loss based on the loaded values 
        self.min_validation_loss = np.min(self.validation_loss_values)
        self.min_validation_loss_epoch = np.argmin(self.validation_loss_values) + 1 # add 1, since index zero corresponds to 1 epoch trained.

        # set the minimum training loss based on the loaded values 
        self.min_training_loss = np.min(self.training_loss_values)
        self.min_training_loss_epoch = np.argmin(self.training_loss_values) + 1 # add 1, since index zero corresponds to 1 epoch trained.
    ...
    def delete_prev_checkpoint(self, prev_checkpoint_epochs, epochs_per_checkpoint_retained=EPOCHS_PER_CHECKPOINT_RETAINED):
        ...
        # if the prev checkpoint to delete is the current validation loss minimum, don't delete
        if prev_checkpoint_epochs == self.min_validation_loss_epoch:
            return
    ...
    def log_min_losses(self):
        """
        Writes to a text file the minimum validation loss and the epoch # that that validation loss was achieved at
        """

        curr_val_min = np.min(self.validation_loss_values)
        curr_val_min_epoch = np.argmin(self.validation_loss_values) + 1 # add 1, since index zero corresponds to 1 epoch trained.
        
        curr_train_min = np.min(self.training_loss_values)
        curr_train_min_epoch = np.argmin(self.training_loss_values) + 1 # add 1, since index zero corresponds to 1 epoch trained.

        update_text_file = False

        if curr_val_min < self.min_validation_loss:
            self.min_validation_loss = curr_val_min
            self.min_validation_loss_epoch = curr_val_min_epoch

            update_text_file = True

            # if it's passed a certain point in training, save a checkpoint
            if self.epochs_trained >= EPOCHS_BEFORE_SAVING_BEST_CHECKPOINTS:
                self.checkpoint_model()

        if curr_train_min < self.min_training_loss:
            self.min_training_loss = curr_train_min
            self.min_training_loss_epoch = curr_train_min_epoch

            update_text_file = True

        if update_text_file:

            # save off txt file with curr train/validation loss minimum
            min_loss_filename = f"min_losses.txt"
            with open(os.path.join(self.experiment_path, min_loss_filename), 'w') as f:
                f.write(f"Minimum Training Loss: {self.min_training_loss}\nEpoch of Minimum: {self.min_training_loss_epoch}\n")
                f.write(f"Minimum Validation Loss: {self.min_validation_loss}\nEpoch of Minimum: {self.min_validation_loss_epoch}")
