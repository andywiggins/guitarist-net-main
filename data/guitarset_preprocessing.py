...
from util import crop_or_pad, chunk_arr, torch_to_numpy
import librosa
import crepe

def crepe_compute_f0(audio, f0data=None, sr=SR, frame_rate=FRAME_RATE, conf_thresh=CREPE_CONFIDENCE_THRESH):
    """
    ...
    # returns times, frequencies, confidences, and model output activations
...
def conditioning_to_tab(conditioning, pitch_bins_per_semitone=1):
    """
    Converts synth conditioning to tablature. 

    Parameters
    ----------
    conditioning : np array (batch, frames, num_strings=6, pitch/vel=2) 
        MIDI synth conditioning to be converted to tab
        values in [0,127]
    pitch_bins_per_semitone : int
        if 1, normal tab is output
        if >1, we get classes for stringwise quantized pitch  

    Returns
    ----------
    tab : np array (batch, frames, num_strings=6, fret_classes=21)
        one_hot tablature
        0 = string off
        1 = open string
        2-20 = 19 numbered frets
    """
    num_classes = ((NUM_FRETS + 1) * pitch_bins_per_semitone) + 1

    pitch = conditioning[:,:,:,0]

    # subtract each string's open pitch
    continuous_fret = (pitch - np.array(OPEN_STRING_MIDI_PITCHES)) * pitch_bins_per_semitone # (batch, frames, 6)
    # set continuous fret to -1 (string off) when the pitch is below the string's range
    continuous_fret[np.where(continuous_fret<=0.0)] = -1
    # set continuous fret to -1 (string off) wherever the original pitches were 0
    continuous_fret[np.where(pitch<=0.0)] = -1

    # round and add 1 so that values are in [0,21]
    fret_class = np.round(continuous_fret) + 1
    # zero values exceeding the highest fret class number
    fret_class[np.where(fret_class>=num_classes)] = 0

    # convert to int tensor
    fret_class_int = torch.tensor(fret_class).to(torch.int64)
    # convert to 1-hot numpy arr
    tab = torch_to_numpy(F.one_hot(fret_class_int, num_classes=num_classes)) # (batch, frames, 6, 21)
