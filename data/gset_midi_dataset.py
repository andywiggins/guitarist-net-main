...
class GsetMidiDataset(Dataset):
    ...
    Torch dataset class for loading saved-off GuitarSet audio and midi conditioning
    """
    def __init__(self, name=None, datasets_path=DATASETS_PATH, dtype=DEFAULT_NP_DTYPE, _overfit_test_mode=False):
        """
        ...
        # test synth ability to overfit but making all clips the same
        if _overfit_test_mode:
            for key in self.dataset.keys():
                item0 = self.dataset[key][0] # just the first item
                self.dataset[key][:] = item0
