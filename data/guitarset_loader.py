...
class GuitarSetLoader:
	...
	Class for loading GuitarSet and creating datasets from it.
	"""
	def __init__(self, data_home=GSET_PATH, download=False, use_crepe_f0_labels=USE_CREPE_F0_LABELS, midi_dataset_use_loudness_env=MIDI_DATASET_USE_LOUDNESS_ENVELOPE):
		"""
		...
		self.midi_dataset_use_loudness_env = midi_dataset_use_loudness_env
		
	def split_dataset_by_player_id(self, valid_player_id=4, test_player_id=5):
		"""
		...
		test_player_id : int
        	ID # of player to hold out for validation
        ...
        """
		valid_player_id = "0" + str(valid_player_id)
		test_player_id = "0" + str(test_player_id)

		available_players = ["00", "01", "02", "03", "04", "05"]
			
		if not valid_player_id in available_players:
			print("Invalid GuitarSet player_id...defaulting to splitting out valid player 04.")
			valid_player_id = "04"
		if not test_player_id in available_players:
			print("Invalid GuitarSet player_id...defaulting to splitting out test player 05.")
			test_player_id = "05"

		splits = {	f"train_player-not{valid_player_id}{test_player_id}": [],
					f"valid_player-{valid_player_id}": [],
					f"test_player-{test_player_id}": []
				}

		for track_id in self.tracks.keys():
			if self.tracks[track_id].player_id == valid_player_id:
				splits[f"valid_player-{valid_player_id}"].append(track_id)
			elif self.tracks[track_id].player_id == test_player_id:
				splits[f"test_player-{test_player_id}"].append(track_id)
			else:
				splits[f"train_player-not{valid_player_id}{test_player_id}"].append(track_id)
	...
	def get_audio_and_MIDI_conditioning_from_track(self, track_id, sr=SR, frame_rate=FRAME_RATE, item_dur=ITEM_DUR, omit_incomplete_chunks=OMIT_INCOMPLETE_CHUNKS):
		...
		for string_idx, string_let in enumerate(GUITAR_STRING_LETTERS):
			...
			if self.midi_dataset_use_loudness_env: # use loudness env
				# get string_loudness_arr
				string_loudness_arr = gset_proc.chunk_frame_labels(string_loudness, item_dur=item_dur, frame_rate=frame_rate) # (items, frames)
				
				# store conditioning
				conditioning[:,:,string_idx,0] = midi_pitch_arr
				conditioning[:,:,string_idx,1] = string_loudness_arr

			else: # use onset velocity
		...
		# add tab as track_data
		track_data['tab'] = gset_proc.conditioning_to_tab(conditioning) # shape : (items, frames, num_strings=6, fret_classes=21)
		track_data['quantized_pitch'] = gset_proc.conditioning_to_tab(conditioning, pitch_bins_per_semitone=PITCH_BINS_PER_SEMITONE) # shape : (items, frames, num_strings=6, fret_classes=21)
	...
	def create_and_save_midi_conditioning_datasets_from_splits(self, splits, save_path=DATASETS_PATH, sr=SR, frame_rate=FRAME_RATE, item_dur=ITEM_DUR):
		...
		for split_name, track_list in tqdm_notebook(splits.items()):
			...
			save_name = split_name + ("_crepe" if self.use_crepe_f0_labels else "") + "_" + "gset-midi" + ("-env" if self.midi_dataset_use_loudness_env else "") + f"_{item_dur}s"
			self.save_dataset(dataset, save_name, save_path=save_path)
	...
	def get_audio_and_MIDI_conditioning_from_track_unchunked(self, track_id, sr=SR, frame_rate=FRAME_RATE):
		...
		for string_idx, string_let in enumerate(GUITAR_STRING_LETTERS):
			...
			if self.midi_dataset_use_loudness_env: # use loudness env
				...
				conditioning[:,string_idx,1] = string_loudness # range (0,127)
				...
				# store conditioning
				conditioning[:,string_idx,0] = midi_pitches
				conditioning[:,string_idx,1] = onset_vel # range (0,127) 

		
		# store track data in dict
		track_data['conditioning'] = np.clip(conditioning, 0,127) # shape : (frames, num_strings=6, pitch_or_onset=2)
