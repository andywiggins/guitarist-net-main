...
class HarmonicOscillator(nn.Module):
    ...
    def forward(self, p, string_idx=None):
        ...
        p : dict
            ...
            p['harm_distr'] : harmonic distribution tensor (batch, # harmonics, frames)
        ...
        if self.inharmonicity == None:
            harmonic_mults = harmonic_mults[None, :, None] # shape: (batches=1, num_osc, frames=1)

        # adjust harmonic mults with 1 global inharm beta
        elif self.inharmonicity == "global": # replace harmonic_nums with values with inharmonicity factored in
        ...
        # mask and then normalize before applying overall amplitude 
        antialias_mask = (freqs < (self.sr / 2)) # bool: (batch, num_osc, frames)
        amplitudes = harm_distr * antialias_mask #  shape: (batch, num_oscillators, frames)
        amplitudes /= amplitudes.sum(1, keepdim=True) # sum over the num_osc dim (1)
        amplitudes *= overall_amplitude[:, None, :] # (batch, num_osc, frames)
